...
$app->get('/administracion', function() use ($app) {
    ...
    if (!isset($_SESSION['usuarioLogin'])) {
        ...
        if($_SESSION['usuarioLogin']['es_admin']==0 && $_SESSION['usuarioLogin']['es_moderador']==0 && $_SESSION['usuarioLogin']['es_redactor']==0){
            $app->redirect($app->router()->urlFor('inicio'));
            die();
        }else{
            $req = new comun();
...
$app->get('/administracion/nuevanoticia', function() use ($app) {
    if (!isset($_SESSION['usuarioLogin'])) {
        ...
        if($_SESSION['usuarioLogin']['es_admin']==0 && $_SESSION['usuarioLogin']['es_moderador']==0 && $_SESSION['usuarioLogin']['es_redactor']==0){
            ...
            $rep = $req->compruebaReportes();
            $ret = $req->compruebaRetos();
            $app->render('admin/nuevanoticia.html.twig', array('repNum' => $rep,
                'usuarioLogin' => $_SESSION['usuarioLogin'],
                'retNum' => $ret));
            die();
        }
...
$app->get('/administracion/equipos', function() use ($app) {
    if (!isset($_SESSION['usuarioLogin'])) {
        ...
        if($_SESSION['usuarioLogin']['es_admin']==0 && $_SESSION['usuarioLogin']['es_moderador']==0 && $_SESSION['usuarioLogin']['es_redactor']==0){
            ...
            $app->render('admin/listaEquipos.html.twig', array('equipos' => $equipos,
                'repNum' => $rep,
                'usuarioLogin' => $_SESSION['usuarioLogin'],
                'retNum' => $ret));
            die();
